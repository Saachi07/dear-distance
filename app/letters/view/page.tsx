...
function LetterContent() {
    ...
    useEffect(() => {
      if (initiallyOpened) {
        try {
          ...
          setDecryptedContent("Error decrypting content. Check ENCRYPTION_KEY.");
        }
      }
      ...
      <>
        ...
        {!isOpened ? (
          <EnvelopeAnimation 
            ...
            onOpen={handleOpen}
            isUnlocked={fullData.is_unlocked}
          />
        ) : (
    ...
    const fetchLetter = async () => {
      try {
        const { data, error: fetchError } = await supabase
          .from('letters')
          .select(`
            id, title, content_encrypted, sender_id, recipient_id, created_at,
            open_when_condition, password_hash, scheduled_reveal_at, is_unlocked, opened_at,
        ...
        if (fetchError) {
          setError(fetchError.message || 'Letter not found')
          return
        }
        ...
        setLetter({ ...rawData, author: authorProfile } as FullLetter)
      } catch (err) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-soft-pink via-white to-dusty-blue px-4">
        <div className="text-center">
          ...
          <p className="text-red-500 mb-6 bg-white/50 p-4 rounded-lg">{error || 'Letter not found'}</p>
          <Link href="/letters" className="inline-flex items-center gap-2 px-6 py-3 bg-rose-gold text-white rounded-lg font-semibold hover:bg-rose-gold/90 transition-all">
            <ArrowLeft className="w-5 h-5" /> Back to Letters
          </Link>
        </div>
      </div>
    )
  ...
  return (
    <div className="container mx-auto max-w-4xl">
      <Link href="/letters" className="mb-4 inline-flex items-center gap-2 text-vintage-ink/70 hover:text-vintage-ink transition-colors">
        <ArrowLeft className="w-5 h-5" /> Back to Letters
      </Link>
      <DisplayLetter letter={formattedLetter} fullData={letter} />
    </div>
  )
}
